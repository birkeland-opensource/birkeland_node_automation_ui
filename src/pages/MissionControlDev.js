...
import {
  ...
  get_rebalance_fee,
  get_wallet_info,
} from "../services/api/lightning_node_communication_service";
import {
  extract_fee_earned_channel_opening_cost,
  getDateRange,
  get_fee_earned_from_forwards,
...
const MissionControlDev = () => {
  ...
  const [channel_info_with_accounting, setchannel_info_with_accounting] =
    useState(null);
  const [channle_opening_fee_earned, setChannle_opening_fee_earned] =
    useState(null);
  ...
  React.useEffect(() => {
    const fetchDataAsync = async () => {
      ...
      if (resp?.success) {
        ...
        let fee_and_cost = extract_fee_earned_channel_opening_cost(
          resp?.message?.channels
        );
            ...
            <table className="table">
              ...
              {channel_info_with_accounting?.map((item, index) => (
                <tr className="first_tr" key={index + 1}>
                  ...
                  <td className="gradient_text">
                    {Number(item?.remote_balance)?.toLocaleString()}
                  </td>

                  {/* <td>{(item?.remote_balance / (item?.local_balance+item?.remote_balance))*100}</td> */}
                  <td>{Number(item?.local_balance)?.toLocaleString()}</td>
                  <td>
                    {Number(item?.total_satoshis_received)?.toLocaleString()}
                  </td>
                  ...
                  <td>
                    ...
                    <input
                      type="range"
                      className="w-100 range-input1"
                      defaultValue={Math.floor(
                        (Number(item?.remote_balance) /
                          (Number(item?.remote_balance) +
                            Number(item?.local_balance))) *
                          100
                      )}
                      style={{
                        background: `linear-gradient(to right, #FEA321 0%, #FEA321 ${Math.floor(
                          (Number(item?.remote_balance) /
                            (Number(item?.remote_balance) +
                              Number(item?.local_balance))) *
                            100
        ...
        <div className="container1">
          ...
          <div className="pending-item">
            <div className="img-box">
              ...
              <p>
                {Number(
                  wallet_balance_info?.unconfirmed_balance
                )?.toLocaleString()}{" "}
                Sats
                <br /> Pending Deposit
              </p>
            ...
            >
                  ...
                  <table className="w-100">
                    ...
                    <tr>
                      ...
                      <td className="text-right">
                        {Math.floor(fee_earned?.fee_earned)?.toLocaleString()}
                      </td>
                    </tr>
                    <thead>
                      <tr>
                    ...
                    <tr>
                      ...
                      <td className="text-right">
                        {rebalance_cost?.total_fee_in_sats?.toLocaleString()}
                      </td>
                    </tr>
                    <tr>
                      <td>Open Channels</td>
                    ...
                    <tr>
                      ...
                      <td className="text-right">
                        {channel_closing_info?.closing_fee?.toLocaleString()}
                      </td>
                    </tr>
                    <tr>
                      <td>Total</td>
                ...
                <div className="box box3">
                  <table className="w-100">
                    ...
                    <tr>
                      ...
                      <td className="text-right">
                        {fee_earned?.amount_forwarded?.toLocaleString()}
                      </td>
                    </tr>
                    <tr>
                      <td>Transaction</td>
              ...
              <GridItem colSpan={1} rowSpan={1}>
                <div className="box box4">
                  Balance:{" "}
                  {Number(wallet_balance_info?.total_balance)?.toLocaleString()}{" "}
                  sats <br />
                  Deposit: - sats <br />
                  Growth: - YTD
                </div>
